...
function (configure_crx_template TEMPLATE_DIR EXT_DIR)
    ...
    foreach(TemplateFile ${TEMPLATELIST})
        ...
        if (CUREXT STREQUAL ".png" OR CUREXT STREQUAL ".ico" OR CUREXT STREQUAL ".jpg" OR IS_DIRECTORY ${TemplateFile})
            if (VERBOSE)
                message(STATUS "Copying extension template file ${CURFILE}")
            endif()
            file(COPY "${SOURCE_FILE}" DESTINATION "${EXT_DIR}/")
        else()
            if (VERBOSE)
                message("Configuring extension template file ${CURFILE}")
            endif()
...
function (configure_extension_wyrmhole BROWSER)

    set (EXT_DIR "${CMAKE_CURRENT_BINARY_DIR}/${BROWSER}Extension")
    message(STATUS "${BROWSER} extension (unpacked) will be built in ${EXT_DIR}")
    if (NOT EXISTS ${EXT_DIR})
        file(MAKE_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}/${BROWSER}Extension")
    endif()
    set (TEMPLATE_DIR "${FB_ROOT}/crx_template")
    configure_crx_template("${TEMPLATE_DIR}" "${EXT_DIR}")

    set (TEMPLATE_OVERRIDE_DIR "${CMAKE_CURRENT_SOURCE_DIR}/extension_template")
    if (EXISTS "${TEMPLATE_OVERRIDE_DIR}")
        message(STATUS "${TEMPLATE_OVERRIDE_DIR} found; using custom files to override default crx files")
        configure_crx_template("${TEMPLATE_OVERRIDE_DIR}" "${EXT_DIR}")
    else()
    ...
    set (TEMPLATE_OVERRIDE_DIR "${CMAKE_CURRENT_SOURCE_DIR}/extension_template_${BROWSER}")
    if (EXISTS "${TEMPLATE_OVERRIDE_DIR}")
        message(STATUS "${TEMPLATE_OVERRIDE_DIR} found; using browser specific custom files to override default crx files")
        configure_crx_template("${TEMPLATE_OVERRIDE_DIR}" "${EXT_DIR}")
    else()
        message(STATUS "${TEMPLATE_OVERRIDE_DIR} does not exist; using default crx files")
    endif()
...
endfunction(configure_extension_wyrmhole)
